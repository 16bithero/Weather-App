...
export default class Forecast extends Component {
    ...
    getWeatherData = () => {
        axios.get(`https://api.openweathermap.org/data/2.5/forecast?q=${this.search}&appid=${API_KEY}`)
        .then(res =>  { 
            console.log(res.data)
            this.setState({...this.state, weather: res.data,
        ...
        return (
            <>
              <form className="form__group field">
                <input onChange={this.readCity} type="input" className="form__field" placeholder="Change City" name="city" id='name' required />
                <label className="form__label">Change City</label>
                <button onSubmit={this.getWeatherData} className="bttn">Search</button>
            </form>
            <br></br>
